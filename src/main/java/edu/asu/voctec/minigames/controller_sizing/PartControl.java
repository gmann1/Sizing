...
public abstract class PartControl extends BasicComponent {

	protected boolean mouseIsPressed = false;
	protected static PartControl objectSelected;
	
	public PartControl(Image image, int x, int y) {
		super(image, x, y);
	}
    ...
    {
        return (y >= ControllerSizingPart3.getBatteryBankAreaBounds().y 
        		&& y <= (ControllerSizingPart3.getBatteryBankAreaBounds().y+ControllerSizingPart3.getBatteryBankAreaBounds().height) 
        		&& x >= ControllerSizingPart3.getBatteryBankAreaBounds().x 
        		&& x <= (ControllerSizingPart3.getBatteryBankAreaBounds().x+ControllerSizingPart3.getBatteryBankAreaBounds().width));
    }
