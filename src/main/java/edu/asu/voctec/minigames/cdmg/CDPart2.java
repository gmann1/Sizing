...
public class CDPart2 extends GameTemplate
{
		...
		protected void actionPerformed()
		{
			if (correctAnswer)
			{
				...
				if (TaskScreen.currentImage < 2)
				{
					...
					TaskScreen.currentImage = 2;
				}
				try
				{
					Game.updateExitText(
							"Good Job!",
							"You have successfully completed both parts of the Critical Design Month game",
	...
	public void init(GameContainer container, StateBasedGame game) throws SlickException
	{
		...
		instructionBox
				.setText("Determine the tilt and orientation. Click the arrows to adjust tilt and orientation of the solar panel for the critical design month.");
		this.backgroundImage = new Image(BACKGROUND);
		topText.setText("Rarotonga, Cook Islands. "
				+ "Latitude: 12.2 degrees S, Longitude: 159.8 degrees W");
		BasicComponent compass = new BasicComponent(new Image(COMPASS), 205 + 248 - 35,
				516 - 59);
		...
		criticalMonth = new BasicComponent(new Image(CRITICAL_MONTH_IMAGE),
				hintBox.getX(), hintBox.getY());
		criticalMonth.rescale(instructionBox.getBounds().width,
				instructionBox.getBounds().height);
		...
		TextField orientation = new TextField(new Rectangle(322 - 35, 462, 500, 500),
				0.95f, "Orientation:                    ",
		...
		angle = new TextField(new Rectangle(322 - 35, 526, 500, 500), 0.95f,
				"Angle:           " + s + "�ｽ", TextDisplay.FormattingOption.CLIP_TEXT);
		angle.setFontSize(LARGE_FONT_SIZE);
		angle.setFontColor(FONT_COLOR);
		
		// arrows
		
		rotateArrow = new Button(new Image(ROTATE_ARROW), 158 - 100 + 10, 486,
				new Rectangle(0, 0, 50, 50), null);
		...
		genericHints.add("Best tilt angle for all year is latitude plus 15 degrees.");
		genericHints
		...
		backButton.addActionListener(new TransitionButtonListener(CDPart1.class));
		
		/*
		 * BasicComponent dot = new BasicComponent(new
		 * Image("resources/default/img/minigames/criticalDesign/sunBeam.png"), 100,100);
		 * dot.rescale(.3f); dot.setX(152); dot.setY(382);
		 * 
	...
	private void displayHint(int hCount, boolean readyClick)
	{
		...
		if (hCount == 0)
		{
			...
			System.out.println("Generic Hint1 shown, total hints: " + CDPart1.hints);
		}
		if (hCount == 1)
		{
			if (readyClick)
			...
			System.out.println("Generic Hint2 shown, total hints: " + CDPart1.hints);
		}
		if (hCount > 1)
		{
			Game.getCurrentTask().getCurrentAttempt().addHints(1);
			...
			System.out.println("User requested more hints, none to give. total hints: "
					+ CDPart1.hints);
			...
			throws SlickException
	{
		...
		if (!nextState)
		{
			...
			if (lc == 5)
			{
				try
				{
					...
					Game.getCurrentGame().addState(new BatteryIntro(),
		...
		if (container.getInput().isMouseButtonDown(Input.MOUSE_LEFT_BUTTON))
		{
			...
			if ((MouseX >= upArrow.getX() && MouseX <= (upArrow.getX() + upArrow
					.getBounds().width)) // check if X is within range
					&& (MouseY >= upArrow.getY() && MouseY <= (upArrow.getY() + upArrow
							.getBounds().height)))
			{
				arrowChange = true;
			...
			else if ((MouseX >= downArrow.getX() && MouseX <= (downArrow.getX() + downArrow
					.getBounds().width)) // check if X is within range
					&& (MouseY >= downArrow.getY() && MouseY <= (downArrow.getY() + downArrow
							.getBounds().height)))
			{
				arrowChange = true;
