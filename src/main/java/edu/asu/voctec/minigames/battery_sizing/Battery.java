...
public class Battery extends BatteryControl{
    ...
    {
    	BasicComponent TransparentBattery = new BasicComponent(BatteryGameScreen.getTransparentBatteryImage(),xBatteryOffset+(x*xBatteryIntervals),yBatteryOffset+(y*yBatteryIntervals));
    ...
    {
    	BasicComponent parallelLines = new BasicComponent(BatteryGameScreen.getVerticalLinesImage(),xLinesOffset+(x*xBatteryIntervals),yLinesOffset+(y*yBatteryIntervals));
    ...
    public static boolean isGameOver()
    {
    	if(!batteryArray.isEmpty())
    	{
    		...
    		{
    			if(getTotalCapacity(batteryArray.get(0))>= BatteryGameScreen.getRequiredCapacity() && getTotalCapacity(batteryArray.get(0))<= 1.5*BatteryGameScreen.getRequiredCapacity() && getTotalVoltage()== BatteryGameScreen.getRequiredVoltage())
    				return true;
    		}
    	} 
    	return false;
