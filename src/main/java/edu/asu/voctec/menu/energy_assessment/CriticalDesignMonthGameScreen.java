...
public class CriticalDesignMonthGameScreen extends Menu
{
	...
	private Panel panel;
	private Sun sun;
	private int timer = 0; //in miliseconds
	...
	public void adjustScore(double angleDelta)
	{
		...
		System.out.println("Hit: \n\tAngle: " + (Math.abs(180 - angleDelta) / 90)
				+ "\n\tScore: " + scoreIncrease);
	...
	public void fireBeam() throws SlickException
	{
		sunBeams.add(new SunBeam((int) sun.getX() + sun.getImage().getWidth(), 
				(int) sun.getY() + sun.getImage().getHeight(), 
				new Random().nextInt(801), 600, BEAM_SPEED) );
	}
			...
			throws SlickException
	{
		...
		for (SunBeam beam : sunBeams)
		{
			...
			{
				Rectangle beamRectangle = new Rectangle(
						(int) beam.getX(), (int) beam.getY(), 
						beam.getImage().getWidth(), beam.getImage().getHeight());
				Rectangle panelRectangle = new Rectangle(
						(int) panel.getX(), (int) panel.getY(), 
						panel.getCurrentImage().getWidth(), panel.getCurrentImage().getHeight());
				
				// Check collision with panel
				if(beamRectangle.intersects(panelRectangle))
				{
					this.adjustScore(Math.abs(beam.getAngle() - panel.getAngle()));
					beam.delete();
				}
