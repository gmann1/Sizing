...
public class GameTemplate extends GUI
{
	...
	public void intializeDefaults() throws SlickException
	{
		...
		topText = new TextAreaX(textLocation, new Rectangle(0, 0, 410, 150), null);
		...
		instructionBox = new TextAreaX(hintBounds, relativeHintTextBounds, null);
		...
		// Back Button
		backButton = new Button(new Image(ImagePaths.BACK_BUTTON), 5, 5, new Rectangle(0,
				0, 50, 25), "Back");
		...
		readyButtonOff = new Button(new Image(ImagePaths.CONTINUE_BUTTON_OFF),
				sidePanel.getX() + sidePanel.getBounds().width / 2
						- UtilFunctions.getImageBounds(readyButtonImage).width / 2,
				hintBox.getY() + hintBox.getBounds().height + 50, textBounds, "Ready");
		...
		textBounds = UtilFunctions.dialateRectangle(textBounds, 0.80f);
		continueButton = new Button(continueButtonImage, sidePanel.getX()
				+ sidePanel.getBounds().width / 2
				- UtilFunctions.getImageBounds(readyButtonImage).width / 2,
				readyButton.getY() + 73, textBounds, "Continue");
		...
		hintButton = new Button(hintButtonImage, readyButton.getX() + 18,
				readyButton.getY() - textBounds.height - 5, textBounds, "Click for Hint");
		
		hintButton.setX(sidePanel.getX() + sidePanel.getBounds().width / 2
				- hintButton.getBounds().width / 2);
		hintButton.setY(401);
	...
	public void continueButtonOn()
	{
		...
		{
			continueButton
					.setCurrentImage(new Image(ImagePaths.CONTINUE_BUTTON_ON), true);
		}
	...
	public void continueButtonOff()
	{
		...
		try
		{
			continueButton.setCurrentImage(new Image(ImagePaths.CONTINUE_BUTTON_OFF),
					true);
		}
		catch (SlickException e)
		{
	...
	public int initiateStars(int starCount, int sequenceCount) throws SlickException
	{
		...
		if (sequenceCount < 50)
		{
			...
			sDisplay.setY(hintBox.getBounds().height + hintBox.getY() - 35
					- sDisplay.getBounds().height / 2);
			if (sequenceCount == 0)
			{
				if (flare != null)
				{
